...
for (t in (seq(1:ncol(df.copy)))){print(df.copy[,t])
print(df.copy2[,t]) }
}

                  #for (k in seq(1:length(i))){
                    #colnames(df.copy)[k] <- paste( str_trim(df.colname[k]),"Result")}

split.round.columns(chess.data)
...
### Extra code, delete after 
dataframe <- chess.data

colnames(a)[n] 
<- paste(i, "result", sep = "")
<- paste(i, "opponent.ID", sep = "")




chess.data$round.1.result <- 
chess.data$round.1.id <- str_extract(chess.data[,4], "[0-9]{1,}")
