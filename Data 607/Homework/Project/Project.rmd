...
chess.data$Total.points <- str_extract(chess.data[,3], "[[0-9]].[[0-9]]\\s")
#chess.data$Post.score <- str_extract(str_extract(chess.data[,2], ">.[[0-9]]{1,}"), "[[0-9]]{1,}")
dataframe < chess.data
...
split.round.columns <- function(dataframe){
                        ...
                        for (i in df.colname){ 
                            for (j in seq(1:length(dataframe[[i]]))){
                                ...
                                #df.copy3[j,i] <- dataframe$Pre.score[dataframe$ID == str_extract(dataframe[[i]][j], "[[0-9]]{1,}")]
                           }
                         } 
                      colnames(df.copy) <- str_c(str_trim(df.colname), " Results")
                      colnames(df.copy2) <- str_c(str_trim(df.colname), " Opponent")
                      #colnames(df.copy3) <- str_c(str_trim(df.colname), " Opponent Prescore")
                      for (t in (seq(1:ncol(df)))){
                          dataframe <- cbind(dataframe, df.copy[,t, drop = FALSE])
                          dataframe <- cbind(dataframe, df.copy2[,t, drop = FALSE])
                          #dataframe <- cbind(dataframe, df.copy3[, t, drop = FALSE])
                      }
    return(dataframe) 
  }
...
str_extract(chess.data[[3]][4], "[[:alpha:]]")

chess.data


chess.data.players <- chess.data[,c(1:4)]
