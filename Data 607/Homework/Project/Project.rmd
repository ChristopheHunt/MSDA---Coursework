...
test <- function(dataframe){
        colname <- (str_extract(colnames(dataframe), "Round.+"))
        for (i in colname){ 
          for (j in seq(1:length(dataframe[[colname]])))
          p <- str_extract(dataframe[[colname]][j], "[[:alpha:]]")
          print(p)
        }
}
(str_extract(colnames(chess.data), "Round.+"))
test(chess.data)
