...
### Our first task is to convert the a `txt` file to an easier to manipulate data frame. The below image is the current structure of the `txt` file and the below code will convert it to a useable dataframe. 
...
colnames(x) <-  x[1, ]
x = x[-1,]
kable(x) #check dataframe
```

### Now we have the dataframe in a more appropriate format, we can find the mean of the opponents prescores that each player played in the chess tournament. 
...
row.names(ID.AverOPscore) <- NULL
kable(head(ID.AverOPscore))
```

### We now have a reference table  with the IDs and the average prescore of the opponents played. We just need to look at our original dataframe and use the reference table to produce the results in the desired format `Player's Name`, `Player's State`, `Total Number of Points`, `Player's Pre-Rating`, and `Average Pre Chess Rating of Opponents`. 
...
kable(sqldf("SELECT 
                  ...
                  Pre_score `Player's Pre-Rating`, 
                  avg_prescore `Average Pre Chess Rating of Opponents`
              FROM 'chess.data' C LEFT OUTER JOIN 
                   'ID.AverOPscore' I on C.ID = I.ID"))
```
