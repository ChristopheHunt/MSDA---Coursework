...
y2 <- df$x^3
df2 <- as.data.frame(cbind(y2,x))

ggplot() + 
  geom_line(data = df, aes(x, y), color = 'red') +
  geom_line(data = df2, aes(x,y2), color = 'blue') +
  theme_minimal() +
  geom_text(aes(7.5, 500, label="y=x^3")) +
  geom_text(aes(8, 100, label="provided data")) 
```

The data does not support the proportion model. 
