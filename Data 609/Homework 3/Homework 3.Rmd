...
The least-squares fit of $W = kl^3$ is $W = `r round(a, 6)`l^3$. The sum of the squares of the deviations as `r sum(y.y2^2)` so $D = `r D`$. As the largest absoulte deviation is `r round(max(abs(y.y2)),3)`, $c_{max}$ can be bound as follows:

$$D = `r D` \leq c_{max} \leq `r round(max(abs(y.y2)),3)` = d_{max}$$
...
The least-squares fit of $W = klg^2$ is $W = `r round(a, 6)`lg^2$. The sum of the squares of the deviations as `r sum(y.y2^2)` so $D = `r D`$. As the largest absoulte deviation is `r round(max(abs(y.y2)),3)`, $c_{max}$ can be bound as follows:
