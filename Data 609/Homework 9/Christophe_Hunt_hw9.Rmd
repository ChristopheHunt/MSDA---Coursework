...
```{r, tidy=TRUE, cache=TRUE, warning=FALSE, message=FALSE}
library(ggplot2)
...
C1 has probability $p = x$, therefore the probability of C2 is $p=1-x$. 
...
```{r, tidy=TRUE, cache=TRUE, message=TRUE, warning=TRUE}
...
Geometrically we can therefore determine that Rose will play strategy R1 and Colin can play either strategy 1 or 2. 
