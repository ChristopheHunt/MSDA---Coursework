    ...
    for (i in 1:n){
      ...
      if (x >= 0 & x <= probs[[1]]){
        results$`1` <- results$`1` + 1 
                  ...
                  x <= sum(probs[[2]], probs[[1]])){
        results$`2` <- results$`2` + 1 
       } else if (x > sum(probs[[2]], probs[[1]]) & 
                  x <= sum(probs[[3]], probs[[2]], probs[[1]])){ 
        results$`3` <- results$`3` + 1
       } else if (x > sum(probs[[3]], probs[[2]], probs[[1]]) & 
                  x <= sum(probs[[4]],probs[[3]], probs[[2]], probs[[1]])){
        results$`4` <- results$`4` + 1
       } else if (x > sum(probs[[4]],probs[[3]], probs[[2]], probs[[1]]) & x <= 
                  sum(probs[[5]],probs[[4]],probs[[3]], probs[[2]], probs[[1]])){
...
If the waiting time of a person is the time the person stands in a queue - the time from arrival at the lobby until entry into an available elevator - what are the average and maximum times a person waits in a queue? 
