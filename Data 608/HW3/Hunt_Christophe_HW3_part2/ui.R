...
shinyUI(fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      sliderInput("timePeriod", "Time Period", min = min(as.numeric(dfMort$Year)),
                  max = max(as.numeric(dfMort$Year)), 
                  value = c(min(as.integer(dfMort$Year)), max(as.numeric(dfMort$Year))), sep = ""),
