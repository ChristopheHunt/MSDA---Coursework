...
```{r}
a <- (sum(ifelse(prob.y.x$greaterLotArea == 1 & prob.y.x$greaterSalePrice == 1, 1, 0)) /nrow(prob.y.x)) / ((sum(prob.y.x$greaterLotArea) / nrow(prob.y.x)))
a
```
...
#TODO figure out why beta distribution is not working 

```{r, cache=TRUE, eval=TRUE, results='asis'}
library(MASS)
fitdistr(as.numeric(sub.train.df$LotArea), densfun = "beta", start=list(shape1=0.1,shape2=0.1))
```
