...
> Now we can proceed with the model of choice which is the Partial Least Squares
...
cv <- RMSEP(chem.plsFit)
best.dims <- which.min(cv$val[estimate = "adjCV", , ]) - 1
 
chem.plsFit <- plsr(Yield ~ ., data =  data.set$train, ncomp = best.dims)
```

d. Predict the response for the test set. What is the value of the performance metric and how does this compare with the resampled performance metric on the training set?

```{r}
chem.plsPredict <- predict(chem.plsFit, data = data.set$test)
```
...
> We do see that a boxcox transformed variable for Biological Material tops the list signifcantly but we do see many more manufacturing process in the top list than biological. 
