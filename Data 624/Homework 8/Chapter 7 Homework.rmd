...
```{r, cache = TRUE}
#At one point I got the error optimismBoot not found, a reinstall of the caret package resolved the issue. You could also install from github - devtools::install_github('topepo/caret/pkg/caret') - https://github.com/topepo/caret/issues/706
...
knnTune <- train(data.set$train %>% dplyr::select(-Yield), data.set$train$Yield, 
                method = "knn", 
                preProc = c("center", "scale"))

results <- predict(knnTune, data.set$test %>% dplyr::select(-Yield)) %>% cbind(.,data.set$test$Yield) %>% as.data.frame()
colnames(results) <- c("predicted", "actual")
rmse(results$actual, results$predicted)
```
