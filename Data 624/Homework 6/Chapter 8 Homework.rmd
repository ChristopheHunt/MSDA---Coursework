...
plot(wmurders)
```
There is a clear pattern in the data so we will need to apply differencing. Using the $ndiffs()$ method we see that `r ndiffs(wmurders)` diff
...
```{r}
plot(diff(ibmclose, differences = 2))
```
