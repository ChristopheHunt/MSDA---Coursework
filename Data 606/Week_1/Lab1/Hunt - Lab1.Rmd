---
title: "Introduction to data - Christophe Hunt"
output:
...
setwd("C:/Users/Christophe/Documents/R/Lab1")
source("./more/cdc.R")
```
...
```{r cases1, eval = TRUE}
paste("There are ", length((cdc$genhlth)), "cases in the dataset")
```

How many variables?  
```{r variables, eval = TRUE}
paste("There are", length(names(cdc)), "variables")
```
For each variable, identify its data type (e.g. categorical, discrete).
```{r variables2, eval = TRUE}
str(cdc)
summary(cdc)
```

> genhlth is categorical, exerany is discrete, hlthplan is discrete, smoke100 is discrete, height is continious, weight is continous, wtdesire is continous, age is continous but appears to be measured as discrete, and gender is discrete. 
...
data.frame(table(cdc$height))
data.frame(table(cdc$age))
summary(cdc$height)
summary(cdc$age)
table(cdc$gender)/length(cdc$gender)
table(cdc$exerany)/length(cdc$exerany)

genders <- data.frame(table(cdc$gender)/length(cdc$gender))
paste("The number of males in the sample is", length(cdc$gender) *  genders$Freq[genders$Var1 == "m"] )

health <- data.frame(table(cdc$genhlth)/length(cdc$genhlth))
paste("The proportion of the sample reporting being in excellent health is", health$Freq[health$Var1 == "excellent"])
...
> This boxplot shows how feelings of health may be correlated with BMI which seems intuitively to make sense. 
...
> I chose the exercise variable to see if there was an correlation with BMI and self-reporting that a perons exercises. I think this variable will have a link to BMI because I would assume that if a respondent is reporting that they exercise then they would, in general, have a lower BMI. The figure does suggest that those reporting that they exercise have a lower BMI. I was curious because this is self reporting there is the possiblity that those that self-report exercise that they may be lying and you might not actual see a relationship. 
...
library(ggplot2)
ggplot(cdc, aes(x=cdc$weight, y = cdc$wtdesire)) +  geom_point(shape=1)
```

> The relationship between the two variables is a positive correlation, 
higher the weight the heigher the desired weight. I believe this indicates 
that people maintain a goal weight based on their current weight and height
and there is not on single goal weight. 
...
> wdiff is continious and its variance data. If the an observation of wdiff is 0 
then the person weights their desired weight. If wdiff is positive then the person
would like to gain weight, whereas if wdiff is negative then the person would like 
to lose weight. 
...
library(ggplot2)
ggplot(cdc, aes(wdiff)) +  geom_density() 
summary(cdc$wdiff)

```

> wdiff has a center at -10 lbs, it has a left skew which is expected that more people 
want to lose weight than gain it. This tells us that in general more people would like 
to lose weight than people would like to gain weight. 

-   Using numerical summaries and a side-by-side box plot, determine if men tend
    to view their weight differently than women.
    
```{r onyourown4, eval = TRUE}
summary(subset(cdc$wdiff, cdc$gender == "m"))
summary(subset(cdc$wdiff, cdc$gender == "f"))

library(ggplot2)
ggplot(cdc, aes(cdc$gender, wdiff, fill = cdc$gender)) + geom_boxplot()

```

> It does appear that men tend to report that they wish to lose less weight than 
the females surveyed. The difference is slight and it would be interesting to 
know if the difference was statisically significant. 
...
```{r onyourown5, eval= TRUE}
mean(cdc$weight)
sd(cdc$weight)
x <- length(subset(cdc$weight, cdc$weight < mean(cdc$weight) + sd(cdc$weight) & 
                     cdc$weight > mean(cdc$weight) - sd(cdc$weight) )) / length(cdc$weight)

paste("The proportion of values that are within one standard deviation of the mean is", x)
