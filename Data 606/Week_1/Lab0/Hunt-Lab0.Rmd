...
If we wanted to connect the data


points with lines, we could add a third argument,
...
```{r question-2, eval=TRUE, message=FALSE}
library(ggplot2)
plot <- ggplot(arbuthnot, aes(year, girls)) + geom_point() + geom_smooth()
plot
```
--
> There does appear to be a an upward trend after 1660. It appears that something external may have negatively influenced the birth rate until 1660 and then it was removed and allowed the increase in birth rates. Ultimately, I assume that some external factors have an influence on the change of birthrates because of the dramatic change. 
...
the future.
```{r question-3, eval = TRUE, message = FALSE}
x <- arbuthnot$year
y <- (arbuthnot$boys / (arbuthnot$boys + arbuthnot$girls))
df <- data.frame(x,y)
plot <- ggplot(df, aes(x, y)) + geom_point() + geom_smooth() + ylim(0,1)
plot
```
--
> It appears that the proportion of boys over this period of time is slightly higher than 50%. 
...
x <- present[order(-present$year),]
paste("The data set has observations of", length(x$year),"years, between", tail(x$year, n = 1), "and", head(x$year , n = 1))
x$year
``` 
...
```{r on onyourown3, eval = TRUE}
x <- data.frame(dim(present))
y <- data.frame(dim(arbuthnot))
paste("The count of observations for present are", x[1,] , "and the count of observations for arbuthnot is", y[1,])
paste("The count of variables for present are", x[2,] , "and the count of variables for arbuthnot is", y[2,])

```

-   Are they on a similar scale?

> The two data sets are not on a similar scale, they use a different time frame and different samples. arbuthnot is using data from one city within England whereas present is using an entire country's data.  
...
```{r onyourown4, eval = TRUE, message = FALSE}
x <- present$year
y <- (present$boys / (present$boys + present$girls))
df <- data.frame(x,y)
plot <- ggplot(df, aes(x, y)) + geom_point() + suppressWarnings(geom_smooth()) + ylim(0,1)
plot
```
---

> I see that proportionally boys are born at a rate greater than 50% but only slightly greater. 

--- 

> Looking at the arbuthnot data set we do see similar results for the proportion of boys born to girls. 

```{r onyourown4a, eval = TRUE, message = FALSE}
x <- arbuthnot$year
y <- (arbuthnot$boys / (arbuthnot$boys + arbuthnot$girls))
df <- data.frame(x,y)
plot <- ggplot(df, aes(x, y)) + geom_point() + geom_smooth() + ylim(0,1)
